...
"""
뉴스 모니터링 대시보드
실시간 뉴스 분석 결과를 시각화하고 모니터링하는 웹 대시보드
"""
...
from news_trading_integration import NewsTradingIntegration, TradingSignal
...
class NewsMonitoringDashboard:
    ...
    def __init__(self, config_file: str = "config/news_trading_config.json"):
        ...
        self.trading_system = NewsTradingIntegration(config_file)
        self.dashboard_data = None
        self.is_running = False
        
        # Flask 앱 초기화
        self.app = Flask(__name__)
        self.app.config['SECRET_KEY'] = 'news_monitoring_secret_key'
        ...
        logger.info("뉴스 모니터링 대시보드 초기화 완료")
    ...
    def _setup_logging(self):
        ...
        # 로그 파일 설정
        log_file = f"{log_dir}/dashboard_{datetime.now().strftime('%Y%m%d')}.log"
    ...
    def _setup_routes(self):
        ...
        def get_trading_signals():
            """거래 신호 API"""
            try:
                signals = self._get_latest_trading_signals()
    ...
    def _update_dashboard_data(self):
        ...
        try:
            # 뉴스 수집 및 분석
            stock_analysis = self.trading_system.collect_and_analyze_news()
            ...
            # 거래 신호 생성
            signals = self.trading_system.generate_trading_signals(stock_analysis)
            
            # 상위 종목 추출
            top_stocks = []
            for stock_code, analysis in stock_analysis.items():
                if analysis.investment_score > 50:  # 50점 이상만
                    top_stocks.append({
            ...
            recent_news = []
            for stock_code, analysis in stock_analysis.items():
                for news in analysis.recent_news[:3]:  # 최근 3개 뉴스
                    recent_news.append({
                        "stock_code": stock_code,
                        "stock_name": analysis.stock_name,
                        "title": news,
            ...
            # 시장 감정 점수 계산
            total_sentiment = sum(analysis.sentiment_score for analysis in stock_analysis.values())
            market_sentiment = total_sentiment / len(stock_analysis) if stock_analysis else 0.0
            
            # 위험 알림 생성
            risk_alerts = []
            for stock_code, analysis in stock_analysis.items():
                if analysis.risk_level == "High":
                    risk_alerts.append(f"{analysis.stock_name}: 높은 위험도 감지")
                if analysis.investment_score < 20:
                    risk_alerts.append(f"{analysis.stock_name}: 낮은 투자 점수")
            
            # 대시보드 데이터 생성
            self.dashboard_data = DashboardData(
                timestamp=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                total_news=sum(analysis.news_count for analysis in stock_analysis.values()),
                analyzed_stocks=len(stock_analysis),
                trading_signals=len(signals),
                top_stocks=top_stocks,
                recent_news=recent_news[:20],  # 최근 20개 뉴스
    ...
    def _get_latest_trading_signals(self) -> List[Dict]:
        """최신 거래 신호 조회"""
        try:
            # 최신 거래 결과 파일 찾기
            trading_dir = "data/trading_results"
            if not os.path.exists(trading_dir):
                return []
            
            files = [f for f in os.listdir(trading_dir) if f.startswith("trading_signals_")]
            if not files:
                return []
            
            # 가장 최신 파일 읽기
            latest_file = max(files)
            file_path = os.path.join(trading_dir, latest_file)
...
<head>
    ...
    <title>뉴스 기반 주식 모니터링 대시보드</title>
...
<body>
    <div class="container">
        <div class="header">
            <h1>📊 뉴스 기반 주식 모니터링 대시보드</h1>
            <p style="text-align: center; color: #666;">실시간 뉴스 분석 및 거래 신호 모니터링</p>
        </div>
        ...
        <div class="status-bar">
            ...
            <div class="status-item">
                <div class="status-value" id="trading-signals">-</div>
                <div class="status-label">거래 신호</div>
            </div>
            <div class="status-item">
    ...
    <script>
        ...
        function updateDashboard(data) {
            ...
            document.getElementById('trading-signals').textContent = data.trading_signals || 0;
            ...
            container.innerHTML = stocks.map(stock => `
                <div class="stock-item">
                    ...
                    <div class="stock-score">
                        ...
                        <div class="status-label">${stock.recommendation}</div>
                    </div>
                </div>
            `).join('');
        }
        ...
        function updateRecentNews(news) {
            ...
            container.innerHTML = news.map(item => `
                <div class="news-item">
                    ...
                    <div class="news-meta">${item.stock_name} | ${item.timestamp}</div>
                </div>
            `).join('');
        }
    ...
    logger.info(f"대시보드 템플릿이 생성되었습니다: {template_path}")

def main():
    """메인 함수"""
    import argparse
    
    parser = argparse.ArgumentParser(description="뉴스 모니터링 대시보드")
    ...
    # 대시보드 실행
    dashboard = NewsMonitoringDashboard(args.config)
